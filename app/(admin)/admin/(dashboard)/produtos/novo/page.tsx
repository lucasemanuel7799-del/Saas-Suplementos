...
export default function NewProductPage() {
  ...
  useEffect(() => {
    async function initData() {
      ...
      if (store) {
        setStoreId(store.id);
        const { data: products } = await supabase.from("products").select("category").eq("store_id", store.id);
        if (products) {
    ...
    <div className="max-w-6xl mx-auto pb-20 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Adicionar Produto</h1>
            <p className="text-zinc-400 text-sm mt-0.5">Preencha as informações para o catálogo.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* === COLUNA ESQUERDA (IMAGEM + ESTOQUE + BOTÃO) === */}
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          {/* BOTÃO SALVAR (Agora aqui embaixo) */}
          <button
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className={cardStyle}>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
               ...
               <div>
                  <label className={labelStyle}>Descrição Rápida (Opcional)</label>
                  <input 
                    type="text"
                    value={description}
                    onChange={(e) => setDescription(e.target.value)}
                    className={inputStyle}
                    placeholder="Detalhes chave do produto..."
                  />
               </div>
             </div>
          </div>

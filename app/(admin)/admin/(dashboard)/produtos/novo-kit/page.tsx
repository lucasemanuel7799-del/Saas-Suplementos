...
export default function NewKitPage() {
  ...
  // 1. Busca o ID da Loja (UUID) inicial
  useEffect(() => {
  ...
  // 2. Busca de Produtos (Debounce)
  useEffect(() => {
    const delayDebounce = setTimeout(async () => {
        if (searchTerm.length > 1 && storeId) {
            const { data } = await supabase
                .from("products")
                .select("id, name, image_url, price, cost_price")
                .eq("store_id", storeId) // CORRIGIDO: Busca produtos da loja certa
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!storeId) return toast.error("Loja não identificada. Recarregue.");
    ...
    try {
        ...
        if (imageFile) {
            ...
            const fileName = `kits/${Date.now()}.${fileExt}`;
        ...
        // 1. Criar Produto Pai (Kit)
        const productPayload = {
            store_id: storeId, // UUID da loja
            name: formData.name,
        ...
        // 2. Salvar Itens do Kit na tabela relacional
      ...
      <div className="flex items-center justify-between mb-6 shrink-0">
        <div className="flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-zinc-400 text-sm">Agrupe produtos para aumentar seu ticket médio.</p>
            </div>
        </div>
        <button onClick={handleSave} disabled={isSaving || !storeId} className="hidden sm:flex items-center gap-2 bg-purple-600 hover:bg-purple-500 text-white px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-purple-900/20 transition-all active:scale-95 disabled:opacity-50">
                ...
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 flex flex-col items-center">
                    ...
                    {imagePreview ? (
                        ...
                        <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full aspect-square border-2 border-dashed border-zinc-700 hover:border-purple-500 hover:bg-zinc-800/50 rounded-xl flex flex-col items-center justify-center gap-4 transition-all group">
                            <div className="h-16 w-16 bg-zinc-800 rounded-full flex items-center justify-center text-zinc-500 group-hover:text-purple-500 transition-all"><Upload size={32} /></div>
                            <div className="space-y-1 text-center"><p className="font-bold text-zinc-300 text-xs">Adicionar Imagem</p></div>
                        </button>
                    )}
                </div>
                ...
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 space-y-4">
                    ...
                    <div className="space-y-4">
                         <div className="space-y-1">
                            <label className="text-[10px] font-bold text-zinc-500 uppercase">Estoque de Kits</label>
                        ...
                        <div className="space-y-1">
                            <label className="text-[10px] font-bold text-emerald-500 uppercase">Preço Final (R$)</label>
                            <input required type="text" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} placeholder="0,00" className="w-full bg-zinc-950 border border-emerald-500/30 rounded-lg p-3 text-white focus:border-emerald-500 outline-none font-bold text-xl" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="lg:col-span-2 space-y-6">
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="md:col-span-2 space-y-2">
                            <label className="text-[10px] font-bold text-zinc-500 uppercase">Nome do Kit</label>
                            <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Ex: Kit Verão 30 Dias" className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-lg text-white focus:border-purple-500 outline-none transition-all" />
                        </div>
                        <div className="space-y-2">
                         ...
                         <div className="space-y-2">
                            ...
                            <div className="relative">
                                <input value={formData.barcode} onChange={e => setFormData({...formData, barcode: e.target.value})} placeholder="EAN..." className="w-full bg-zinc-950 border border-zinc-800 rounded-lg pl-9 pr-3 py-3 text-white focus:border-purple-500 outline-none font-mono" />
                ...
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 space-y-6 min-h-[400px]">
                    <div className="flex justify-between items-center">
                        <h2 className="text-sm font-bold text-zinc-300 uppercase flex items-center gap-2 text-[10px]">
                            <Box size={16} className="text-orange-500"/> Composição do Combo
                        </h2>
                        <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-400 bg-zinc-950 px-3 py-1.5 rounded-lg border border-zinc-800">
                            <TrendingUp size={14} className="text-emerald-500" />
                            CUSTO TOTAL: <span className="text-white">R$ {totalCost.toFixed(2).replace('.', ',')}</span>
                        </div>
                    </div>
                    ...
                    <div className="relative">
                        ...
                        <input 
                            ...
                            className="w-full bg-zinc-950 border border-zinc-700 rounded-xl pl-10 pr-4 py-3 text-white focus:border-orange-500 outline-none"
                        />
                                ...
                                {searchResults.map(prod => (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-[10px] text-zinc-500 font-mono">Custo: R$ {prod.cost_price?.toFixed(2)}</p>
                                        </div>
                    ...
                    <div className="space-y-3">
                        {selectedItems.length === 0 ? (
                            <div className="text-center py-10 border-2 border-dashed border-zinc-800 rounded-xl text-zinc-600">
                                <PackagePlus size={40} className="mx-auto mb-2 opacity-20"/>
                                <p className="text-xs font-bold uppercase tracking-widest">Kit Vazio</p>
                            </div>
                        ) : (
                            selectedItems.map(item => (
                                <div key={item.id} className="flex items-center gap-4 bg-zinc-950 p-3 rounded-xl border border-zinc-800 group hover:border-zinc-600 transition-all">
                                    ...
                                    <div className="flex-1">
                                        <p className="font-bold text-white text-xs uppercase">{item.name}</p>
                                        <p className="text-[10px] text-zinc-500 font-mono">Unit: R$ {item.cost_price?.toFixed(2)}</p>
                                    </div>
                                    ...
                                    <button type="button" onClick={() => removeItem(item.id)} className="p-2 text-zinc-700 hover:text-red-500 transition-colors">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            ))
                ...
                <button onClick={handleSave} disabled={isSaving || !storeId} className="sm:hidden w-full flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-500 text-white px-6 py-4 rounded-xl font-bold transition-all disabled:opacity-50">
